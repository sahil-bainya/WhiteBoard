...
var ShapeHandleOverlayUtil_exports = {};
__export(ShapeHandleOverlayUtil_exports, {
  ShapeHandleOverlayUtil: () => ShapeHandleOverlayUtil
});
module.exports = __toCommonJS(ShapeHandleOverlayUtil_exports);
var import_editor = require("@tldraw/editor");
class ShapeHandleOverlayUtil extends import_editor.OverlayUtil {
  ...
  getOverlays() {
    ...
    for (const handle of handles) {
      if (handle.type === "virtual" && vertexHandles.some((vertexHandle) => import_editor.Vec.Dist(handle, vertexHandle) < minDist)) {
        continue;
      }
  ...
  getGeometry(overlay) {
    ...
    const pagePoint = import_editor.Mat.applyToPoint(transform, { x: handle.x, y: handle.y });
  ...
  render(ctx, overlays) {
    ...
    for (let i = overlays.length - 1; i >= 0; i--) {
      ...
      if (handle.type === "clone") {
        const fr2 = 3 / zoom;
        const sideIndex = import_editor.SIDES.indexOf(handle.id);
...
//# sourceMappingURL=ShapeHandleOverlayUtil.js.map
